#!/usr/bin/env python3
"""Kernal CLI agent — runs on Client 1's (target's) machine.
...
2. Register the tunnel URL with the Kernal host via PATCH /api/jobs/{id}/tunnel.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Kernal CLI agent — Client 1 side")
    parser.add_argument("--host", required=True, help="Kernal host base URL (e.g. https://kernal.example.com)")
